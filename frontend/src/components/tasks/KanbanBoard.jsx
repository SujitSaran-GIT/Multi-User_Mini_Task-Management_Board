...
const KanbanBoardInner = ({ columns, tasks, onTaskDrop, onCardDragStart }) => (
  <motion.div
  ...
  >
    {columns.map((col) => (
      <KanbanColumn key={col.id} column={col} tasks={tasks.filter(t => t.status === col.id)} onTaskDrop={onTaskDrop} onCardDragStart={onCardDragStart} />
    ))}
  </motion.div>
);
...
export default function KanbanBoard({ tasks = [], loading }) {
  ...
  return (
    <div className="w-full">
      ...
      <KanbanBoardInner columns={columns} tasks={tasks} onTaskDrop={moveTask} onCardDragStart={handleCardDragStart} />
    </div>
  );
}